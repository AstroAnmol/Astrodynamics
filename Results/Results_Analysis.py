...
# %%
data=pd.read_csv("Trial_0_file.csv")
##
Earth=pd.read_csv("Earth_orbit_file.csv")
Mars=pd.read_csv("Mars_orbit_file.csv")
# %%
E_R1=np.array(Earth['Position_1 (km)'])
E_R2=np.array(Earth['Position_2 (km)'])
E_R3=np.array(Earth['Position_3 (km)'])
# %%
M_R1=np.array(Mars['Position_1 (km)'])
M_R2=np.array(Mars['Position_2 (km)'])
M_R3=np.array(Mars['Position_3 (km)'])
# %%
R1=np.array(data['Position_1'][:-4])
R2=np.array(data['Position_2'][:-4])
R3=np.array(data['Position_3'][:-4])
# Velocity
...
#Adjoint 
AR1=np.array(data['APosition_1'][:-4])
AR2=np.array(data['APosition_2'][:-4])
AR3=np.array(data['APosition_3'][:-4])
...
plt.plot(R1,R2, ls='-.', label='Spacecraft')
plt.legend()
plt.axis('square')
